...
export default {
  ...
  mixins: [mixin],
  asyncData ({store}) {
    return store.dispatch('FETCH_MESSAGE', { accesstoken: store.state.loginUser.token})
  },
  computed: {
    message () {
    ...
    currentData () {
      return this.selectItem === 1 ? this.message.hasnot_read_messages : this.message.has_read_messages
    }
  },
  methods: {
    // 切换tab
    ...
    // 标记所有为已读
    markall() {
      fetchPost('/message/mark_all', 'accesstoken=' + this.userInfo.token).then((res) => {
        if (res && res.success) {
